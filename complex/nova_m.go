...
func calcNova(width, height, maxiter uint, palette [][3]byte, img *image.RGBA) {
	var zy0 float64 = -1.50

	for y := uint(0); y < height; y++ {
		var zx0 float64 = -1.50
		for x := uint(0); x < width; x++ {
			i := nova(1, 0, zx0, zy0, maxiter)
			...
			zx0 += 3.0 / float64(width)
		}
		//fmt.Println(zy0)
		zy0 += 3.0 / float64(height)
	}
}
...
func main() {
	...
	calcNova(uint(width), uint(height), uint(maxiter), mandmap[:], img)

	outfile, err := os.Create("nova_m.png")
	if err != nil {
		panic(err)
	}
	defer outfile.Close()
