...
func nova(zx0, zy0, cx, cy float64, maxiter uint) uint {
	/*
		const Epsilon = 0.001
...
func calcNova(width, height, maxiter uint, palette [][3]byte, img *image.RGBA, cx, cy float64) {
	var zy0 float64 = -2.00

	for y := uint(0); y < height; y++ {
		var zx0 float64 = -2.00
		for x := uint(0); x < width; x++ {
			i := nova(zx0, zy0, cx, cy, maxiter)
			// guard palette indexing
			idx := (3 * i) % uint(len(palette))
...
func main() {
	if len(os.Args) < 4 {
		println("usage: ./newton width height maxiter")
		os.Exit(1)
	}
	...
	for frame := range 10 {
		...
		cx := -float64(frame) / 10.0
		calcNova(uint(width), uint(height), uint(maxiter), mandmap[:], img, 0.0, cx)

		outfile, err := os.Create("nova" + strconv.Itoa(frame) + ".png")
		if err != nil {
			panic(err)
		}
		defer outfile.Close()
