...
"""Interactive renderer of the classic Barnsley J2 fractal based on Pygame."""
...
TITLE = "Interactive Barnsley Julia J2 renderer"
SCREEN_WIDTH = 600
SCREEN_HEIGHT = 300
IMAGE_WIDTH = 256
IMAGE_HEIGHT = 256
...
def recalc_barnsley_m2(image, palette, xmin, ymin, xmax, ymax, maxiter=100):
...
def barnsley_j2(zx0, zy0, cx, cy, maxiter):
    """Calculate number of iterations for given complex number to escape from set."""
    c = complex(cx, cy)
    z = complex(zx0, zy0)
    for i in range(maxiter):
...
def recalc_barnsley_j2(image, palette, xmin, ymin, xmax, ymax, cx, cy, maxiter=1000):
    """Recalculate the whole fractal and render the set into given image."""
    width, height = image.get_size()  # rozmery obrazku
    stepx = (xmax - xmin) / width
    stepy = (ymax - ymin) / height
    ...
    for y in range(height):
        x1 = xmin
        for x in range(width):
            i = barnsley_j2(x1, y1, cx, cy, maxiter)
            i = 3 * i % 256
            color = palette[i][2] + (palette[i][1] << 8) + (palette[i][0] << 16)
            image.set_at((x, y), color)
            x1 += stepx
...
def main():
    ...
    recalc_barnsley_m2(image1, palette_blues.palette, XMIN, YMIN, XMAX, YMAX, MAXITER)
    event_loop(display, image1, image2, clock, recalc_barnsley_j2)
