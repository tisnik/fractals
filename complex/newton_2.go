...
func newton(cx, cy float64, maxiter uint) uint {
	zx := cx
	zy := cy
	i := uint(0)
	...
	for i < maxiter {
		...
		if math.Hypot(zx-zxn, zy-zyn) < 0.001 {
			return i
		}
		zx = zxn
		zy = zyn
...
func calcNewton(width, height, maxiter uint, palette [][3]byte, img *image.RGBA) {
	var cy float64 = -2.00

	for y := uint(0); y < height; y++ {
		var cx float64 = -2.00
		for x := uint(0); x < width; x++ {
			i := newton(cx, cy, maxiter)
...
func main() {
	...
	calcNewton(uint(width), uint(height), uint(maxiter), mandmap[:], img)

	outfile, err := os.Create("newton.png")
	if err != nil {
		panic(err)
	}
	defer outfile.Close()
