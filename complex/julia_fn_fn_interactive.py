"""Interactive renderer of the Julia fractal fn||fn based on Pygame."""
...
def recalc_mandelbrot_fn_fn(image, palette, xmin, ymin, xmax, ymax, maxiter=100):
    ...
    for y in range(height):
        ...
        for x in range(width):
            i = mandelbrot_fn_fn(x1, y1, maxiter)
            i = 3 * i % 256
...
def julia_fn_fn(zx0, zy0, cx, cy, maxiter):
...
def recalc_julia_fn_fn(image, palette, xmin, ymin, xmax, ymax, cx, cy, maxiter=1000):
    """Recalculate the whole fractal and render the set into given image."""
    width, height = image.get_size()  # rozmery obrazku
    stepx = (xmax - xmin) / width
    stepy = (ymax - ymin) / height
    ...
    for y in range(height):
        x1 = xmin
        for x in range(width):
            i = julia_fn_fn(x1, y1, cx, cy, maxiter)
            i = 3 * i % 256
            color = palette[i][2] + (palette[i][1] << 8) + (palette[i][0] << 16)
            image.set_at((x, y), color)
            x1 += stepx
...
def main():
    ...
    recalc_mandelbrot_fn_fn(image1, palette_blues.palette, XMIN, YMIN, XMAX, YMAX, MAXITER)
    event_loop(display, image1, image2, clock, recalc_julia_fn_fn)
