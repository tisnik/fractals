...
func hline(bitmap [][]float64, x1, x2, y int, d float64) {
	fromX := min(x1, x2)
	toX := max(x1, x2)
...
func vline(bitmap [][]float64, x, y1, y2 int, d float64) {
	fromY := min(y1, y2)
	toY := max(y1, y2)
...
func line(bitmap [][]float64, x1 int, y1 int, x2 int, y2 int, d float64) {
	dx := abs(x2 - x1)
	dy := abs(y2 - y1)
	sx := 1
	sy := 1
...
func calcSystem(width, height, maxiter int) *image.RGBA {
	...
	for range maxiter {
		line(bmp, rand.IntN(width), rand.IntN(height), rand.IntN(width), rand.IntN(height), 1.0)
		line(bmp, rand.IntN(width), rand.IntN(height), rand.IntN(width), rand.IntN(height), -1.0)
	}
